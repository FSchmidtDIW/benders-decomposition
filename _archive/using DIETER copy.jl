...
settings = (year_set = [2014,2016],
            ...
            parameters = (
                ...
                acc_tup = (range = [1e-1,1e-8],int = :log),
...
function solve_adaptive_subproblem(sub_m::JuMP.AbstractModel,res,opt_gap::Float64,threshold::Float64;solver=Gurobi.Optimizer)
    temp_mod = copy(sub_m) # copy model so as to not alter the default subproblem
    set_silent(temp_mod)
    # define complicating variables
    capa = filter(x->contains(string.(x),"N_"),all_variables(temp_mod))
    ...
    function absolute_stopping_criterion(cb_data,cb_where)
        ...
        push!(c,objbound)
        if maximum(c) > thr
            GRBterminate(backend(temp_mod).optimizer.model.inner)
        end
        return obj 
    end
    MOI.set(temp_mod,Gurobi.CallbackFunction(),absolute_stopping_criterion)
    ...
    if termination_status(temp_mod)!=OPTIMAL
        if termination_status(temp_mod) == MOI.INTERRUPTED
            @info "inexact oracle - skipping cut"
            return (obj = nothing, duals = nothing)
        else
            @error "Subproblem did not solve to optimality!"
            return temp_mod
        end
...
function dynamic_par_adjustment!(iter::Int64,method::Val{:lvl},stab_params::Dict,upper_bound,lower_bound,lower_bound_stabilised,incumbency_count,serious_count)
    ...
    k = iter
    if stab_params[:serious_step][k]
        stab_params[:v_lvl][k+1] = (1-stab_params[:beta])*(upper_bound-lower_bound)#min(stab_params[:v_lvl][k],(1-stab_params[:beta])*(upper_bound-lower_bound))
    else 
...
function benders(dtr::DieterModel,
    ...
    for k in 1:settings.parameters.max_iter
        ...
        threshold = 1.6*upper_bound

        # This bit will be parallelised
        for year in settings.year_set
            tmp_dict[year] = solve_adaptive_subproblem(sub_mod_dict[year],res,0.0001,
            #compute_conv_tol(gap,settings.parameters.optimality_gap,settings.parameters.acc_tup)
            threshold;
            solver)
        end
        # save output
        ...
        if settings.multicut_bool
            for s in settings.year_set
                if !isnothing(oracle_output[k][s].obj)
                    cut = @constraint(mstr,
        ...
        if sum(isnothing(oracle_output[k][s].obj) for s in settings.year_set) > 0
            f = upper_bound + 10
        else
        ...
        adjCtr_bool = false
        # serious step test
        if upper_bound - settings.parameters.descent_threshold*v > f
            upper_bound = f
            x_best = res
            push!(stability_centre,x_best)
...
qm, x_best = benders(dtr,settings)
