...
function solve_subproblem(sub_m::JuMP.AbstractModel,res,opt_gap;solver=Gurobi.Optimizer)
    ...
    # Define output
    ret = (obj = objective_value(temp_mod),dual_obj = maximum(c),duals=dual.(temp_mod[:fix]));
    return ret
end
...
function benders(dtr::DieterModel,
    ...
    if settings.dual_subproblems
        ...
        for y in settings.year_set
            ...
            @constraint(tmp,fix[i=eachindex(tmp[:capa])],tmp[:capa][i]==0)
            d_tmp = dualize(tmp,dual_names=DualNames("dual_","dual_"))
    ...
    for k in 1:settings.parameters.max_iter
        ...
        Threads.@threads for year in settings.year_set
        ...
        if settings.multicut_bool
            for s in settings.year_set
                cut = @constraint(mstr,
                                mstr[:θ][s] >= oracle_output[k][s].dual_obj - mstr[:investment_obj] + sum(oracle_output[k][s].duals[i]*(mstr[:capa][i] - res[i] ) for i in eachindex(mstr[:capa]))
                            );
            set_name(cut,"cut$(k)[$s]")
            end
        else
            cut = @constraint(mstr,
                                mstr[:θ] >= sum(probs[s]*(oracle_output[k][s].dual_obj - mstr[:investment_obj] + sum(oracle_output[k][s].duals[i]*(mstr[:capa][i] - res[i] ) for i in eachindex(mstr[:capa]))) for s in settings.year_set)
                            );
            set_name(cut,"cut$(k)")
        end
